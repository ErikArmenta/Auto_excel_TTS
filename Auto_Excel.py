...
def init_supabase() -> Client:
    url = st.secrets.get("SUPABASE_URL", "https://xznmlyflcdqdkdrrcdpq.supabase.co")
    key = st.secrets.get("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inh6bm1seWZsY2RxZGtkcnJjZHBxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODY0NTcxMDIsImV4cCI6MjEwMjAzMzEwMn0.gkzgIhXGuLS-Izk_9v3waewoaP3JwEkW1Z3M-6zvcBE")
...
# --- ESTILOS VISUALES PARA STREAMLIT (Neumorphism / Clean) ---
def apply_status_color(row):
    """Aplica colores a la fila basados en las discrepancias."""
    styles = [''] * len(row)

    # Índices basados en la estructura del DataFrame procesado
    try:
...
if report_files and base_dict:
    for file in report_files:
        ...
        try:
            ...
            # Buscar header real si es necesario (asumimos fila 0 o 4 según tus scripts)
            ...
            df = df[df['CONTAINER'].str.len() == 11] # Filtro estricto de 11 caracteres

            # Limpieza global de "nd"
            df = df.applymap(clean_val)

            # 2. Aplicar Lógica V3 (Cruce y Discrepancias)
            df['BD GATE OUT (Diff)'] = None
            df['AMTC Delivery (Loaded)'] = None
            df['AMTC Unload Notification (Empty)'] = None
            df['BD EMPTY TERM (Diff)'] = None

            # Variables temporales para el coloreado
            df['Base_Gate_Out'] = None
            df['Base_Empty_Term'] = None

            # Manejar variaciones del nombre de la columna de retorno
            ...
            column_mapping = {
                ...
                'TOTAL': 'TOTAL'
                # Agrega las columnas adicionales de precios aquí
            }
            ...
            if st.button(f"Subir datos verificados (WK-{semana})", type="primary", key=f"up_{file.name}"):
                with st.spinner("Sincronizando con Supabase..."):
                    try:
                        ...
                        st.success(f"¡Éxito! {len(records)} registros listos para la base de datos.")
